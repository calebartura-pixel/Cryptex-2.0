...
function showTestNotification() {
    if (Notification.permission === 'granted') {
        const notification = new Notification('Notifications Enabled!', {
            body: 'You will now receive crypto price alerts even when you\'re not viewing this tab.',
...
function showPriceNotification(crypto, currentPrice, condition, targetPrice) {
    if (!checkNotificationSupport() || Notification.permission !== 'granted') {
        return false;
    }
    
    const title = `${cryptoNames[crypto]} Alert!`;
    const body = `Price is now $${currentPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\nYour alert: Price ${condition} $${targetPrice.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
    
    const notification = new Notification(title, {
        body: body,
        icon: 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y="0.9em" font-size="90">💰</text></svg>',
        tag: `price-alert-${crypto}-${Date.now()}`,
...
function checkAlerts() {
    ...
    alerts.forEach((alert, index) => {
        ...
        if (conditionMet) {
            const notificationSent = showPriceNotification(
                alert.crypto,
                currentPrice,
                alert.condition,
                alert.targetPrice
